...
**아래 GT 는 잠정이다.** MCP 도구 39개로 recipe 가 45개가 됐고, 거기에 축 조회
...
(5번 충전소 · recipe_032 대 recipe_033)에서 조회 후보 수는 그대로인데 LLM
...
#
#   001  말한 장소 → 장소 좌표 변환
#   022  말한 장소 → 장소 좌표 변환 → CCTV 조회
#   032  말한 장소 → 장소 좌표 변환 → 전기차 충전소 검색
#   042  말한 장소 → 장소 좌표 변환 → 지점 행정구역 판별 → 연령별 인구 구성 조회
...
UTTERANCES = [
    ...
    (3, "오송역 CCTV 보여줘",        {"recipe_022"}, True),  # 옛 recipe_025
    (4, "청주시 인구 구성 알려줘",   {"recipe_042"}, True),  # 옛 recipe_046 · 045
    (5, "오송역 근처 충전소 찾아줘", {"recipe_032"}, True),  # 옛 recipe_035
