...
UTTERANCES = [
    ...
    (3, "오송역 CCTV 보여줘",        {"recipe_035"}, True),  # 옛 recipe_020 · 021 · 022 · 025
    (4, "청주시 인구 구성 알려줘",   {"recipe_057"}, True),  # 옛 recipe_038 · 039 · 042 · 046 · 045
    (5, "오송역 근처 충전소 찾아줘", {"recipe_045"}, True),  # 옛 recipe_030 · 031 · 032 · 035
    ...
    (10, "오송역이 어느 동인지 알려줘",   {"recipe_033"}, True),  # 옛 recipe_018 · 장소 → 좌표 → 지점 행정구역 판별
    (11, "오송역 행정경계 보여줘",        {"recipe_032"}, True),  # 옛 recipe_017 · 장소 → 좌표 → 행정구역 조회
    (12, "청주시 행정경계 보여줘",        {"recipe_004"}, True),  # 키워드 → 행정구역 조회
    (13, "오송역 국회의원 누구야",        {"recipe_039"}, True),  # 옛 recipe_024 · 장소 → 좌표 → 지점 전체 선거구 판별
    (14, "오송역 국회의원 공약 보여줘",   {"recipe_040"}, True),  # 옛 recipe_025 · 장소 → 좌표 → 선거구 공약 검색
    ...
    (18, "오송역 일대 인구 얼마야",       {"recipe_044"}, True),  # 옛 recipe_029 · 장소 → 좌표 → 인구 통계
    ...
    (20, "대전역 연령대별 인구 알려줘",   {"recipe_057"}, True),  # 옛 recipe_038 · 장소 → 좌표 → 행정구역 판별 → 연령별 인구
    (21, "경부선 선형 데이터 줘",         {"recipe_002"}, True),  # 장소 → 철도 구간 형상
    (22, "오송역 지나는 노선 알려줘",     {"recipe_003"}, True),  # 장소 → 철도 노선 조회
    (23, "경부선 주변 CCTV 보여줘",       {"recipe_048"}, True),  # 옛 recipe_033 · 장소 → 철도 구간 형상 → CCTV
    # 사슬이 넷인 유일한 발화다. 4단이 실제로 서는지를 이 한 줄이 지킨다.
    (24, "오송역 근처 충전소 자세히 알려줘", {"recipe_059"}, True),  # 옛 recipe_040 · 장소 → 좌표 → 충전소 검색 → 상세
    ...
    (30, "오송역 선거구 알려줘",          {"recipe_037"}, True),  # 옛 recipe_022 · 장소 → 좌표 → 지점 선거구 판별
