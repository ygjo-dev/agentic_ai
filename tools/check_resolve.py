...
알아낸 것 (4a55c44 ~ e8fcdc2 · qwen2.5:7b · 따로 적지 않았으면 _init recipe 6)
  ...
  앞토막 recipe 는 판정을 무너뜨린다 (e8fcdc2).
    ...
    앞토막이 다른 모든 recipe 에 포개져 "어디서 끝나는가" 를 못 가른다.
    프롬프트로 절반은 회복되지만 나머지를 못 채우고, 조일수록 끝점을 모호하게
    말하는 발화가 대신 무너진다("…봐줘" 9/10 → 0/5).

  프롬프트를 절차로 나누면 판정이 나아진다.
    규칙 셋을 동시에 저울질하는 대신 순서 있는 단계로 쪼갠 것.
    샘플 셋이 0/10 → 10/10 이 됐고 시연에 안 쓰는 발화도 9/10 → 5/5 로 올랐다.
    다만 recipe 8개에서도 2번 발화를 못 채워 이번에는 되돌렸다.
