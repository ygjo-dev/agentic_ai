...
**아래 GT 는 잠정이다.** MCP 도구 38개로 recipe 가 41개가 됐고, 거기에 축 조회
...
#
#   001  말한 장소 → 장소 좌표 변환
#   021  말한 장소 → 장소 좌표 변환 → CCTV 조회
#   031  말한 장소 → 장소 좌표 변환 → 전기차 충전소 검색
#   039  말한 장소 → 장소 좌표 변환 → 지점 행정구역 판별 → 연령별 인구 구성 조회
...
UTTERANCES = [
    ...
    (3, "오송역 CCTV 보여줘",        {"recipe_021"}, True),  # 옛 recipe_022 · 025
    (4, "청주시 인구 구성 알려줘",   {"recipe_039"}, True),  # 옛 recipe_042 · 046 · 045
    (5, "오송역 근처 충전소 찾아줘", {"recipe_031"}, True),  # 옛 recipe_032 · 035
