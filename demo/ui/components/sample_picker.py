...
recipe 8개가 두 갈래다 — 승강장 CCTV(001·003·005·006) · 궤도 검측차
(002·004·007·008). 각 갈래 안에서는 프레임 추출까지냐 분석까지냐 Word 냐
PPT 냐로 갈린다. 그래서 **무엇으로 시작하는가** 와 **어디서 끝나는가** 를
둘 다 밝혀야 하나로 좁혀진다.
...
#
# **recipe 가 6 → 8 로 늘면서 위 점수가 무너졌다.** 아래 기대값은 같은 경로를
# 가리키도록 번호만 갈아끼운 것이다.
#   2026-08-15 · qwen2.5:7b · _init(recipe 8) · 각 10회 (NO_MATCH 는 5회)
#   0/10 · 0/10 · 0/10 · 5/5
#   1 → {001,003,005,006} · 2 → {002,004} · 3 → {003,005} (10회 모두 같은 답)
# 새로 생긴 001·002(데이터 → 프레임 추출)가 나머지의 앞토막이라 후보가 안
# 갈린다. **발화를 고치는 것은 다음 작업이다.** 자세한 것은 tools/check_resolve.py.
#
# 이 목록이 보여주는 것
#   대상이 갈린다                        승강장 · 궤도
...
#
#   recipe_007 · recipe_008 은 이 목록으로는 한 번도 안 켜진다(궤도 + 문서).
#   새로 생긴 recipe_001 · recipe_002(데이터 → 프레임 추출)도 안 켜진다 —
#   "프레임만 뽑아줘" 에 해당하는 발화가 목록에 없다.
#
# 온톨로지나 menu 가 바뀌면 다시 재야 한다 (tools/check_resolve.py).
SAMPLES = [
    # SELECT : 발화가 Recipe 하나로 유일하게 결정.
    # 분석에서 끝나므로 보고서까지 가는 005/006 은 후보가 아니다.
    ("승강장에 사람이 얼마나 몰렸는지 분석해줘", "SELECT", "recipe_003", ["recipe_003"]),
    # 대상(궤도 검측차)이 갈라서 승강장 갈래는 후보가 아니다.
    ("검측차 영상에서 레일 갈라진 데 있는지 확인해줘", "SELECT", "recipe_004", ["recipe_004"]),
    # CLARIFY : 산출 형식을 안 밝혀 Word / PPT 로 갈린다.
    ("승강장 혼잡도 결과를 문서로 정리해줘", "CLARIFY", None, ["recipe_005", "recipe_006"]),
