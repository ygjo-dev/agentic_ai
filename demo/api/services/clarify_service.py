...
# 스위치의 기본값을 주는 환경변수. "0" 이면 끔. **없으면 켬(지금 길)이다.**
#
# 좁히기(RESOLVE_NARROW)와 반대 방향이다. 그쪽은 아직 화면에 안 나온 새 길이라
# 꺼 두고 켜 보지만, 이쪽은 이미 화면에서 도는 길이라 기본이 켬이다 — 변수가
# 없으면 지금과 한 글자도 달라지지 않는다.
#
# 왜 끌 수 있게 두나. 2026-08-28 하정목 박사님이 메모리를 어떻게 쓸지 정하기
# 전에 세션을 쓰는 것을 넣지 말자고 하셨다. 기능을 지우는 대신 끌 수 있게 했다 —
# 정답표 서른한 발화 중 서른셋 회가 되묻기라, 끄면 그것들이 막다른 길이 된다.
# 메모리 설계가 정해지면 이 자리를 그 설계로 옮긴다.
CHOICE_ENV = "CLARIFY_CHOICE"
...
def enabled() -> bool:
    """되묻기 뒤 고르기를 쓰는가. 환경변수가 "0" 일 때만 끔.

    출력  참이면 지금 길 — 되묻기를 기억해 두고 다음 발화로 고를 수 있음
          거짓이면 「서른넷째」 이전 — 되묻기는 그대로 나오지만 번호를 쳐도
          새 발화로 처리됨. 기억해 두는 일도 안 함
    규칙  변수가 없으면 켬. 있어도 "0" 이 아니면 켬
    제약  끈 상태에서 메모리를 쓰지 않는다.
          끄는 까닭이 세션을 안 쓰는 것이므로, 고르기만 막고 기억해 두면
          끈 것이 아니다. 부르는 자리(execute_service)가 둘 다 건너뛴다
    """
    return os.environ.get(CHOICE_ENV, "") != "0"
