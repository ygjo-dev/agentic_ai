...
- **발화 4 의 `034` · `045` · `046` 이 안 갈린다.** 셋 다 인구이고 셋 다
  `statistics` 를 내놓아 shortlist 가 셋을 다 올리고 LLM 도 셋을 다 고른다.
  앞토막을 57자에서 31자로 줄여도 그대로였다 (2026-08-23 아홉째).
  문장 길이가 아니라 능력이 인접한 것이 원인이다.
- **발화 8 은 `about` 축에서 갈린다.** `"철도 안전 문서 찾아줘"` 의 about 이
  열 번 모두 `group_transport` 로 잡혀 `search_documents`(`group_document`)가
  shortlist 에 못 든다. `description` 이 닿는 자리가 아니다.
  위의 "발화 8 의 기대값이 잠정" 과 함께 정해야 한다.
