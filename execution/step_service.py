...
# ★ **발화에서 인자를 뽑는 자리는 여기 없다.** 발화 해석 LLM 이 argument 로
#   함께 내놓는 것 하나뿐이다. 예전에는 그것이 null 일 때 도는 정규식
#   대비책(place_in)이 여기 있었는데 걷었다 — 두 곳이 인자를 뽑으면 어느 값이
#   어디서 왔는지 표에서 안 갈리고, 정규식은 "충북대 근처" 를 못 잡으면서
#   "지금 화면에 든 읍면동 경계" 의 「읍면동」은 장소로 집었다(실측).
#   같은 일을 다른 이름의 정규식으로 되살리지 않는다.
