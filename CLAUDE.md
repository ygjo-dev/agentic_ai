...
확인이 필요하면 그 작업에서 한 번 재서 `NOTES.md` 에 적는다.

---

## 커밋

- **커밋하지 않는다.** 결과를 보고하고 멈춘다.
- 커밋 메시지는 `NOTES.md` 에 적어둔다. 사람이 화면을 확인한 뒤 직접 커밋한다.
...
작업을 마치면 저장소 루트의 `NOTES.md` 를 이번 작업 내용으로 새로 쓴다.

```
커밋 메시지
결정한 것과 그 이유
건너뛴 것
사람이 눈으로 볼 것      ← 화면을 띄워야만 알 수 있는 것들
지운 것의 목록            ← 삭제가 큰 작업일 때. 나중에 찾을 수 있게
```
...
이 저장소는 **주석이 명세 역할을 한다.** 실측값과 "왜 이렇게 했는가" 가 주석에
있고, 그것이 유일한 기록인 경우가 많다.

- 변경으로 사실과 어긋나게 된 주석은 반드시 고친다. 낡은 주석을 남기지 않는다.
- 없는 파일 · 없는 함수를 가리키는 주석을 만들지 않는다.
- 실측으로 얻은 숫자를 지우지 않는다. 값이 바뀌었으면 다시 재서 갱신한다.
