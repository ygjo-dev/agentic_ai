...
def test_a_node_carries_only_a_name_and_a_description():
    ...
    조용히 어긋날 수 있다 — about 이 붙은 노드에 kind: function 이 적혀 있으면
    어느 쪽이 맞는지 알 방법이 없다.

    inputs / outputs 도 마찬가지다. 그건 관계라서 edges 에 적힌다.
    """
...
def test_a_nodes_character_comes_from_its_relations():
    """세 부류로 갈리고 서로 겹치지 않는다.

    데이터 노드는 받지도 내놓지도 않는다 — 그 자체로 존재하는 것이다.
    그룹 노드도 마찬가지지만 about 의 대상으로 등장한다는 점이 다르다.
    """
...
def test_a_general_node_accepts_subtypes_through_the_is_a_chain():
    """상위 타입 한 줄만 적어도 하위 타입을 받는다. is-a 를 두는 이유다.

    프레임 추출은 "영상" 만 받는다고 적혀 있는데, 승강장 CCTV 영상도 궤도
    검측차 영상도 받는다. 하위 타입이 생길 때마다 기존 노드를 고치지 않아도
    된다는 뜻이다.

    **반대 방향은 안 된다.** "영상" 을 내놓는 노드를 "승강장 CCTV 영상" 만 받는
    노드에 이을 수는 없다 — 그 영상이 승강장 것이라는 보장이 없다.
    """
...
def test_only_concrete_data_can_start_a_path():
    """경로는 **손에 잡히는 데이터**로 시작한다. 형식으로는 시작할 수 없다.

    "영상" 은 형식이지 데이터가 아니다. 그것을 시작점으로 삼으면
    "영상으로 프레임을 추출하고 혼잡도를 분석한다" 는 recipe 가 만들어지는데,
    사람이 그걸 골라도 **어느 영상인지 아무 데도 안 적혀 있다.** 실행하려는
    순간 막히고, 그때는 이미 menu 에 실려 있다.

    조용히 깨지는 자리다 — "받는 것도 내놓는 것도 없는 노드" 로 시작점을 잡으면
    형식 노드가 전부 여기 걸린다.
    """
...
def test_an_is_a_cycle_does_not_hang(isolated_workspace):
    """온톨로지에 순환이 적혀도 화면은 돌아야 한다.

    사람이 손으로 적는 파일이라 순환은 언제든 생긴다. 온톨로지가 잘못 적히는
    것보다 화면이 안 도는 것이 더 나쁘다 — 시연 중에 서버가 멈춘다.
    """
...
def test_nodes_connected_in_a_recipe_become_solid_edges():
    ...
    생긴다. recipe 를 근거로 삼으면 실제로 실행하는 연결만 남는다.

    **라벨이 없다.** 예전에는 두 노드가 무엇을 주고받는지 인터페이스 이름으로
...
def test_only_about_relations_become_dotted_edges():
    """점선은 about 뿐이다. is-a 와 hasInput / hasOutput 은 그리지 않는다.

    셋 다 그리면 17개 노드에 24개 선이 얽혀 무엇이 무엇에 관한 것인지 안 보인다.
    화면이 답해야 하는 질문은 "이 경로가 무엇에 관한 것인가" 하나다.
