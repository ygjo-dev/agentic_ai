...
# 마지막으로 성공한 /screen 응답을 여기 둔다. 백엔드가 잠깐 끊겨도
# 그래프가 사라지지 않아야 한다.
SCREEN_CACHE_KEY = "_screen_cache"
...
def get_screen() -> tuple[dict, bool]:
    """화면이 그리기 전에 받아 두는 것.
    ...
    """
    try:
        payload = _call("GET", "/screen", timeout=SCREEN_TIMEOUT)
    except ApiError:
        cached = st.session_state.get(SCREEN_CACHE_KEY)
        if cached is None:
            raise
        return cached, True

    st.session_state[SCREEN_CACHE_KEY] = payload
