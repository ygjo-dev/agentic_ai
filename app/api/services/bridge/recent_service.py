...
# "**1. 장소 좌표 변환**" 으로 적는다. 줄머리에 공백이 없다.
#
# **별 둘은 있을 수도 없을 수도 있다.** 저쪽 화면이 마크다운으로 그려서
# 단계 제목을 굵게 감쌌는데(2026-09-02. workflow_answer.STEP_HEAD), 이름 없이
# step_line 을 부르는 자리는 그대로 "1. " 로 적는다. 둘 다 걸려야 한다.
#
# 되묻기 답의 후보 줄("  1  전기차 충전소 검색")과 갈라야 한다. 그쪽은 앞에
# 공백이 있고 번호 뒤에 마침표가 없다.
#
# **한 단계가 여러 줄일 수 있다.** 값 줄("- **면적** : …")과 knowledge.query 의
# 문서 조각 줄이 그것이다 (2026-08-26 「서른한째」). 그 줄들은 이 꼴에
# 안 걸리므로 바로 앞 단계에 붙인다 — 떼어 내면 화면에서 조각이 사라진다.
_STEP_HEAD = re.compile(r"^\*{0,2}\d+\. ")
