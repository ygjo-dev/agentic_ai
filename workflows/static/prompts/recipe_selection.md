...
- reason 다음 candidate_recipe_ids 와 status 와 recipe_id 를 아래 [규칙] 대로 쓴다.
  여기까지가 고르는 일이고, 아래는 발화에서 값을 뽑는 일이다.
- 그다음 argument 를 쓴다. 발화가 무엇으로 시작하느냐에 따라 뽑을 것이 다르다.
...
- argument 다음 travel_mode 와 minutes 를 쓴다. 발화가 말한 것만 쓰고 나머지는 null 이다.
    ...
    admin_level  도·특별시·광역시 -> 시도, 시·군·구 -> 시군구, 읍·면·동 -> 읍면동.
                 그 낱말 뒤에 무엇이 붙든 발화에 있으면 그 층위를 쓴다
                 ("어느 구야" -> 시군구, "무슨 면이야?" -> 읍면동).
                 발화에 그 낱말이 없으면 null 이다
