"""대상 : app/ui/components/flow.py — 흐르는 표시를 얹는 문서

브라우저를 못 연다(chromium · playwright · selenium 없음, 새 의존성 금지).
그래서 **문서에 규칙과 선택자가 들어 있는지** 를 본다. 실제로 움직이는지는
사람이 눈으로 봐야 한다.

정지 그림이 안 바뀌었다는 것은 tests/app/ui/graph_svg/test_path_flow.py 가 본다
(PNG 가 한 바이트도 안 다름).
"""

from app.ui.graph_svg.dot import FLOW_CLASS as DOT_FLOW_CLASS
from app.ui.graph_svg.dot import HIGHLIGHT_COLOR, PATH_PENWIDTH
from app.ui.components import flow, focus_panel, graph_section
...
def test_both_sides_agree_on_the_class_name():
    ...
    flow.py 는 app/ui/graph_svg 를 import 하지 않는다(계층). 대신 이 검사가
    두 값을 붙잡는다.
    """
