"""대상 : app/ui/components/zoom.py — 고른 경로에 화면을 맞추는 문서
...
배율이 실제로 칸에 드는지는 tests/app/ui/graph_svg/test_autofit_scale.py 가
완성된 SVG 로 잰다. 여기는 문서만 본다.
"""

from app.ui.graph_svg.dot import FLOW_CLASS as DOT_FLOW_CLASS
from app.ui.graph_svg.dot import HIGHLIGHT_COLOR
from app.ui.components import flow, focus_panel, graph_section, zoom
...
def test_the_top_gets_the_same_script_but_has_nothing_to_fit():
    ...
    tests/app/ui/test_path_flow_document.py 가 같은 것을 지킨다.
    """
    from app.ui.graph_svg.dot import build_dot
...
def app_documents() -> list[str]:
    ...
    app = AppTest.from_file(str(REPO_ROOT / "app" / "ui" / "main.py"), default_timeout=180)
    app.run()
