...
def future_input(spoken):
    """새 이름으로 배선된 블록 하나를 채운 결과의 input."""
    return workflow_materializer.workflow_of(FUTURE, spoken, NOW)["workflow"]["steps"][0]["input"]


def test_a_new_spoken_name_binds_without_a_line_of_python_for_it():
    """이름마다 분기가 없다. 응답에 칸이 늘면 배선할 수 있는 이름도 함께 는다.

    이름 하나에 파이썬 if 가 붙는 순간 그 이름의 뜻이 게시된 배선표가 아니라
    materializer 에 생긴다. 그러면 recipe 를 고쳐도 실행이 안 따라온다.
    """
    workflow_materializer.validate(copy.deepcopy(FUTURE), "recipe_x")

    assert future_input({"foo": "말한 값", "travel_mode": "도보"}) == {"x": "말한 값", "mode": "WALK"}


def test_a_value_that_was_not_said_falls_back_to_the_published_default():
    """기본값은 게시된 execution 이 갖는다. 안 말한 값을 해석이 지어내지 않는다."""
    assert future_input({}) == {"x": "기본", "mode": "TRANSIT"}


def test_a_block_cannot_read_the_selection_itself_as_a_said_value():
    """고른 것과 그 까닭은 도구 칸에 실을 값이 아니다. 배선이 그것을 가리키면 게시 오류다."""
    for name in ("recipe_id", "status", "reason", "candidate_recipe_ids", "paths"):
        block = copy.deepcopy(FUTURE)
        block["workflow"][0]["input"]["x"] = {"from": f"spoken.{name}", "default": "기본"}
        with pytest.raises(workflow_materializer.PlanError, match="발화가 말한 값의 이름"):
            workflow_materializer.validate(block, "recipe_x")
