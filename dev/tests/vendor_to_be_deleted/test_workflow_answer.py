...
서비스 · 그리기 테스트를 늘리지 말라는 규칙의 밖이다. 이 파일은 그 계층이
아니고 입출력이 문자열뿐이라 구조가 바뀌어도 따라다니지 않는다.
"""
...
#
# Gateway 직접 호출 실물이다 (user_context 는 legacy_vendor.USER_CONTEXT).
...
def clarify(steps, unwired=(), reason=""):
    ...
    return workflow_answer.unresolved_answer(resolved, {"steps": steps, "unwired": list(unwired)})


def no_match(reason="", topics=("철도",), starts=("장소 이름",)):
    resolved = {"status": "NO_MATCH", "candidate_recipe_ids": [], "reason": reason}
    return workflow_answer.unresolved_answer(resolved, {"topics": list(topics), "starts": list(starts)})
...
def unready(status, missing, paths=None, recipe_id="recipe_x"):
    ...
    return workflow_answer.unready_answer(materialized, paths)
...
def test_the_screen_guard_names_what_is_missing_in_the_order_it_was_given():
    ...
    assert answer.splitlines() == [
        workflow_answer.NO_CONTEXT_ANSWER["map_extent"],
        workflow_answer.NO_CONTEXT_ANSWER["point"],
    ]
...
def test_nothing_to_call_says_so(status):
    assert unready(status, []) == workflow_answer.NO_TOOL_ANSWER
