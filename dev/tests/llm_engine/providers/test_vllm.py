...
**요청 모양을 지어내지 않았다.** benchmark 로 검증된 request contract 를 옮긴
것이고, 이 시험이 그 계약을 붙든다. 요청이 달라지면 그 benchmark 측정 결과와
더는 견줄 수 없다.
...
def test_the_request_matches_the_verified_vllm_contract(sent_request):
    """benchmark 를 낸 요청과 같은 요청이어야 함.
...
def test_the_raw_answer_comes_back_untouched(sent_request):
    """response JSON 에서 content 원문만 꺼냄. 파싱은 부르는 쪽이 함.
...
def test_the_model_and_timeout_can_be_swapped_without_restarting(sent_request):
    ...
    call_vllm(
        "발화",
        RESPONSE_SCHEMA,
        config=VllmConfig(model="다른모델", timeout=1, host="http://다른호스트:9"),
    )

    body = json.loads(sent_request["request"].data.decode("utf-8"))
    assert body["model"] == "다른모델"
    assert sent_request["request"].full_url == "http://다른호스트:9/v1/chat/completions"
