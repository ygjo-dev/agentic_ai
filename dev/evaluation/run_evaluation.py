"""벤치마크 한 번을 돌리는 main. 정답표를 읽고 Resolve 를 부르고 채점해 결과를 남긴다.
    ...
    python dev/evaluation/run_evaluation.py --suite dev/evaluation/inputs/test_suites/test_suite_v2.yaml --fan-quiet
                                                                   테스트 세트 v2 · GPU 팬 소음 억제(팬이 오르면 다음 발화 앞에서 기다림)
...
def _measure(plan: list, rows: list, *, planned: int, label_of: dict, resolve, context, materialize: bool, now,
             fan_quiet: bool, recorder, progress, monitor, should_stop) -> tuple[str | None, int, float]:
    """plan 의 (발화, 차례) 를 차례로 재서 rows 에 덧붙임. (멈춘 까닭 또는 None, 이번에 부른 수, 팬 때문에 기다린 초).

    규칙  발화마다 Resolve 를 부르기 앞에서 monitor.before_case(끝난 수, fan_quiet, should_stop). 팬이 높으면 거기서 기다림.
          첫 발화 앞에서도 봄. 마지막 발화 뒤에는 안 봄
          줄마다 차례대로: recorder.case · progress(끝난 수, 전체 수, 그 줄).
          끝난 수는 rows 전체(이어 재면 전에 잰 줄 포함)
          should_stop 은 Resolve 를 부르기 바로 앞에서 봄(팬 기다림 뒤). 참이면 USER_STOP 으로 멈춤.
    ...
    제약  두 줄을 동시에 재지 않는다. 다음 발화를 미리 부르지 않는다. 부르는 중인 Resolve 를 팬 때문에 끊지 않는다
    """
    stopped, called = None, 0
    waited_before = monitor.waited_s if monitor else 0.0
    try:
        for case, number in plan:
            # 다음 요청 앞에서 기다린다. 뒤에서 기다리면 마지막 요청 뒤에도 기다리게 되고,
            # 그만큼은 아무도 기다릴 이유가 없는 시간이다.
            if monitor:
                monitor.before_case(len(rows), fan_quiet=fan_quiet, should_stop=should_stop)
        ...
        stopped = "interrupted"
    return stopped, called, (monitor.waited_s - waited_before) if monitor else 0.0


def _close(head: dict, rows: list, labels: tuple, *, stopped: str | None, complete: bool, elapsed_s: float,
           fan_wait_s: float, ended: datetime.datetime, environment, recorder) -> dict:
    """잰 줄로 결과 한 벌을 세우고 남김. 다 쟀으면 run.json, 아니면 meta.json 만 다시 씀.

    규칙  elapsed_s 는 벽시계로 잰 전체 소요 시간, fan_wait_s 는 그중 팬 때문에 기다린 초의 합.
          추론 시간의 합은 summary.latency.total (발화마다의 timing.resolve_s)
          다 잰 것(complete)만 finished_at 을 적고 recorder.finish 로 run.json 을 씀
    ...
    """
    spent = {"elapsed_s": round(elapsed_s, 1), "fan_wait_s": round(fan_wait_s, 1)}
    tail = {"finished_at": ended.isoformat(), **spent, "stopped": None, "environment": environment}
    if not complete:
        tail = {"finished_at": None, **spent, "stopped": stopped or manage_benchmark.INCOMPLETE,
...
def run(
    ...
    *,
    ...
    출력  {meta, summary, cases}. json 으로 바로 쓸 수 있는 값만
          meta 머리는 monitor_metadata.benchmark_meta. 끝에 finished_at · elapsed_s · fan_wait_s · stopped · environment 를 더함.
    ...
    규칙  only 가 있으면 그 번호만, 없으면 enabled 인 발화만. 파일 차례 그대로
          ...
          fan_quiet_mode 는 GPU 팬 소음 억제. meta.fan_quiet_mode 에 그대로 적히고 monitor 에 넘어감.
          monitor 가 없으면 적혀도 기다리지 않음 (팬을 볼 자가 없음)
          발화마다 Resolve 를 부르기 앞에서 monitor.before_case (팬이 높으면 기다림. 기다린 합은 meta.fan_wait_s)
          발화 하나가 끝날 때마다 차례대로: save_dir 이면 그 줄을 cases.jsonl 에 덧붙임 ·
          progress(잰 수, 전체 수, 그 결과 줄).
    ...
    제약  MCP 도구를 부르지 않는다.
          ...
          resolve 호출 하나만 감싸 재므로 기다려도 시간이 안 늘어나고 판정도 안 바뀜
    ...
    stopped, _called, fan_wait_s = _measure(
        ...
        now=now, fan_quiet=fan_quiet_mode, recorder=recorder, progress=progress, monitor=monitor,
    ...
    return _close(head, rows, labels, stopped=stopped, complete=stopped is None,
                  elapsed_s=(finished - started).total_seconds(), fan_wait_s=fan_wait_s, ended=finished,
...
_SEGMENT_KEYS = ("finished_at", "elapsed_s", "fan_wait_s", "stopped", "stopped_at", "environment", "saved_to")
...
def resume(
    ...
    *,
    ...
    return _close(head, rows, tuple(meta["suite"].get("group_labels") or ()), stopped=stopped,
                  ...
                  elapsed_s=before + (finished - started).total_seconds(), fan_wait_s=waited + fan_wait_s,
                  ended=finished, environment=environment, recorder=recorder)
...
def main() -> int:
    ...
    parser.add_argument(
        "--gpu", choices=("off", "record"), default="record",
        help="record 는 실행 하드웨어(GPU 이름 · VRAM)를 적음, off 는 안 적음 (기본 record)",
    )
    parser.add_argument(
        "--fan-quiet", action="store_true",
        help=f"GPU 팬 소음 억제. 다음 발화 앞에서 팬이 {monitor_gpu.FAN_PAUSE_AT}%% 이상이면 "
             f"모두 {monitor_gpu.FAN_RESUME_AT}%% 이하가 될 때까지 기다림 (기본 꺼짐)",
    )
    ...
    result = run(
        ...
        fan_quiet_mode=args.fan_quiet,
        monitor=monitor_gpu.GpuMonitor() if args.fan_quiet else None,
