"""시연에서 쓸 발화가 지금 SELECT 로 붙는지 재는 도구.

**정답표(check_resolve.UTTERANCES)와 다른 자다.** 정답표는 살아 있는 recipe 와
1대1 이라 「이 recipe 를 어떤 말로도 못 부른다」를 드러내는 자다. 시연 발화는
사람이 실제로 할 말이라 한 recipe 를 여러 말로 부른다 — 같은 recipe 가 여러 줄에
걸린다. 두 성질을 한 자에 담으면 정답표의 1대1 이 깨지므로 파일을 갈랐다.

    .venv/bin/python dev/tools/check_demo.py                  차례대로
...
#
# 이 자는 recipe 를 덮는 자가 아니라 「그날 할 말이 통하는가」를 재는 자다.
# 그래서 같은 recipe 가 여러 줄에 걸리는 것이 정상이고, 발화는 사람이 실제로 할
# 말 그대로 둔다 — 자에 맞추려고 발화를 고치지 않는다.
#
# ★ **4번의 「근처」가 052 가 아니라 060 인 까닭.** 052 는 이름으로 충전소 한
# 곳을 찾아 그 충전기를 보여주고, 060 은 말한 장소를 좌표로 바꿔 그 언저리
# 충전소를 찾은 뒤 그중 하나를 자세히 낸다. 「근처」라고 물었으면 언저리가 다
# 지도에 찍히고 그중 하나가 자세히 나오는 쪽이 옳다. 둘이 갈리는 자리라 사람이
# 화면에서 양쪽 답을 눌러 보고 060 으로 정했다.
DEMO = [
    (1, "오송역 좌표 보여줘", {"recipe_001"}),
    (2, "오송역 CCTV 보여줘", {"recipe_036"}),
    (3, "의왕시 인구 알려줘", {"recipe_012"}),
    (4, "철도기술연구원 근처 충전소 자세히 알려줘", {"recipe_060"}),
