...
UTTERANCES = [
    ...
    (1, "익산역 좌표 알려줘", {"recipe_001"}, True),  # 말한 장소 → 좌표. 좌표까지만
    # ★ 옛 21번은 「경부선 선형 데이터 줘」였다. 「선형 데이터」는 도메인 말이라
    # 사람이 안 쓴다. 002 는 **노선을 지도에 그릴 형상을 내는 유일한 자리**다
    # (003 은 노선·역 목록 · 049 는 CCTV 까지 간다). 1번과 말버릇을 맞췄다
    (2, "경부선 위치 보여줘", {"recipe_002"}, True),
    # ★ 037 을 지워 이제 1대1 이다. 「청주역」으로 쓰려다 되돌렸다 —
    # geo.getRailwayLines(stationName=청주역) 이 0건이다(실측). 천안역은 26건
    (3, "천안역에 무슨 노선 다녀", {"recipe_003"}, True),
    (4, "오송 테스트트랙 교량 화면에 띄워줘", {"recipe_004"}, True),  # 도구가 아니라 화면 명령(digitalTwin.showFacility)이다
    # 함께 걸림 033 · 006 · 024. 033 과 갈리는 근거는 「세종시가 행정구역
    # 이름이다」뿐인데 그 규칙은 사용자에게 안 보인다
    (5, "세종시 행정경계 보여줘", {"recipe_005"}, True),
    (6, "세종시 경계 VWorld 에서 내려받아줘", {"recipe_006"}, True),  # 함께 걸림 005 · 025 · 035. 「VWorld」라는 출처를 대야만 갈린다
    # ★ 「목록으로」를 넣어 015 · 016 · 040 무리에서 뺐다 — 「아흔두째」
    # 갈래 판정표 B 의 손이 그것이었다. election.searchDistricts(sido=대구) 12건
    (7, "대구에 선거구 뭐뭐 있는지 목록으로 줘", {"recipe_008"}, True),
    # ★ 「환경」·「복지」로 쓰려다 되돌렸다 — 그 분야는 0건이다. 실제 분야
    # 여섯은 교통전체·대중교통·도로·선박·철도·항공이고 대중교통은 20건(실측)
    (8, "대중교통 공약 있는 선거구 몇 군데야", {"recipe_009"}, True),
    (9, "국민의힘 당선인이 낸 공약 찾아줘", {"recipe_010"}, True),  # party=국민의힘 20건(실측)
    (10, "시군구 인구 순위 알려줘", {"recipe_012"}, True),  # population.searchStatistics 에 order 칸이 있다
    # ★ 「급속 충전기」로 쓰려다 되돌렸다 — chargerType 은 코드(02 · 04 · 09)를
    # 받아 사람 말로는 못 채운다. query 는 운영기관을 받고 환경부는 나온다(실측).
    # 그래도 함께 걸림 032 · 046 · 052 — 장소를 안 대면 충전소 넷이 다 걸린다
    (11, "환경부가 운영하는 충전소 목록 줘", {"recipe_013"}, True),
    # ★ 「문서에서」가 없으면 007(웹 검색)로 샌다. 지식베이스에 문서는 둘뿐이고
    # 둘 다 철도안전법 계열이다(knowledge.listDocs 실측)
    (12, "철도안전법 조문 문서에서 찾아줘", {"recipe_014"}, True),
    # 함께 걸림 016. 015 는 지역구 데이터셋, 016 은 당선인·공약이 붙은 쪽인데
    # 「정보」로는 안 갈린다. election.getAssemblyDistrict(name=충북 청주서원) 1건
    (13, "충북 청주서원 선거구 정보 줘", {"recipe_015"}, True),
    (14, "청주시 흥덕구 당선인이랑 공약까지 같이 보여줘", {"recipe_016"}, True),  # 「당선인·공약까지」를 말해야 13번에서 갈린다
    (15, "동대구역 둘레 행정경계 띄워줘", {"recipe_033"}, True),  # 함께 걸림 005 · 035. 역 이름이라 005 로는 0건인데 그 규칙이 안 보인다
    (16, "부산역이 무슨 구에 있어", {"recipe_034"}, True),  # 말한 장소 → 좌표 → 지점 행정구역 판별
    (17, "논산시 경계 도형 VWorld 에서 내려받아줘", {"recipe_035"}, True),  # 함께 걸림 006 · 025. 출처 이름을 대야만 갈린다
    (18, "수원역 근처 CCTV 띄워줘", {"recipe_036"}, True),  # 말한 장소 → 좌표 → CCTV
    (19, "광주송정역은 어느 선거구야", {"recipe_038"}, True),  # 「이름만」을 물어 040 · 042 에서 갈린다
    (20, "오송역 둘레 선거구 공약 건수 세줘", {"recipe_039"}, True),  # 함께 걸림 028 · 009 · 040. geocode 의 bbox 가 1.1km 라 사실상 한 선거구다
    (21, "동대구역 지역구 의원 누구야", {"recipe_040"}, True),  # 말한 장소 → 좌표 → 지점 전체 선거구 판별
    (22, "부산역 쪽 의원 공약 뭐 있어", {"recipe_042"}, True),  # 말한 장소 → 좌표 → 그 지점의 선거구 공약
    # 함께 걸림 012. 「일대」가 좌표 길을 가리키지만 지역 이름을 대는 순간
    # 012(키워드 집계)와 갈리기 어렵다
    (23, "오창읍 일대에 사람 얼마나 살아", {"recipe_045"}, True),
    (24, "서대전역 근처에 충전소 있나", {"recipe_046"}, True),  # 함께 걸림 060. 060 은 충전기 상태까지 간다
    # ★ 049 의 menu example(호남선 선로 따라 CCTV 띄워줘)을 안 베끼려고 노선과
    # 어미를 바꿨다. rail.getSectionGeometry(충북선) 1건 · getRailwayLines 24건(실측).
    # 사슬이 셋이다 — 말한 장소 → 구간 형상 → CCTV
    (25, "충북선 따라 CCTV 보여줘", {"recipe_049"}, True),
    (26, "전주시 연령대별 인구 알려줘", {"recipe_058"}, True),  # 사슬 넷 — 장소 → 좌표 → 행정구역 판별 → 연령별 인구
    (27, "군산시 인구 달마다 어떻게 바뀌었어", {"recipe_059"}, True),  # 사슬 넷 — 마지막이 인구 추세
    # ★ 060 의 menu example 을 안 베끼려고 다시 썼다. 사슬이 넷인 두 자리 중
    # 하나다. 함께 걸림 046 · 052
    (28, "광주송정역 옆 충전소에 충전기 비어 있는지까지 알려줘", {"recipe_060"}, True),
    # 함께 걸림 013 · 056 · 060. ev.getStation 은 statId 하나를 받는데 발화에
    # 그 말이 없다. ev.searchStations(query=탄방) 18건(실측)
    (29, "탄방동 충전소 충전기 상태까지 알려줘", {"recipe_052"}, True),
    ...
    (36, "여기 인구 늘고 있어 줄고 있어", {"recipe_055"}, True),  # 사슬 셋 — 마지막이 인구 추세

    # ── 보이는 범위 아홉 (37~45) ──────────────────────────────────────
    #
    # 시작 데이터가 visible_extent 인 것. 지도 문맥이 와야 닿는다.
    #
    # ★ **아홉이 다 「지금 보이는」이나 「현재 화면」으로 시작한다.**
    # 위와 같은 까닭이고 여기는 더 좁다 — SCREEN_WORDS 여덟 중 보이는 범위로
    # 읽히는 것이 그 **둘뿐**이다. 「화면에 보이는」·「화면에 걸친」은 부분
    # 문자열로 안 걸려 화면 recipe 를 아예 못 본다(실측으로 확인했다).
    # ★ 「아흔두째」 보고가 걱정한 「menu 신호어를 베껴서 맞을 위험」이 이 자리인데,
    # **못 바꾼다.** 바꾸면 못 잰다. 신호어를 발화가 아니라 문맥에서 읽게 하는
    # 것이 진짜 답이고 그것은 이번 범위가 아니다 — 「열린 과제」에 적었다.

    (37, "지금 보이는 범위 행정경계 그려줘", {"recipe_024"}, True),  # 함께 걸림 025. 025 와 「VWorld」 한 낱말로만 갈린다
    (38, "지금 보이는 범위 경계 VWorld 로 받아줘", {"recipe_025"}, True),  # 함께 걸림 024 · 006
    (39, "지금 보이는 데 CCTV 다 띄워줘", {"recipe_026"}, True),  # 보이는 범위 → CCTV
    (40, "지금 보이는 데 철도 노선 뭐뭐 있어", {"recipe_027"}, True),  # 003 은 이름을 대야 하고 여기는 화면이라 갈린다
    (41, "지금 보이는 선거구 공약 건수 세줘", {"recipe_028"}, True),  # 함께 걸림 009 · 039. 「건수 세줘」가 셋에 다 맞는다
    (42, "지금 보이는 데 의원 공약 검색해줘", {"recipe_029"}, True),  # 보이는 범위 → 선거구 공약 검색
    (43, "현재 화면 인구 알려줘", {"recipe_031"}, True),  # 보이는 범위 → 인구 통계
    (44, "지금 보이는 데 충전소 찾아줘", {"recipe_032"}, True),  # 함께 걸림 013 · 056
    (45, "현재 화면 충전소 충전기 상태까지 줘", {"recipe_056"}, True),  # 함께 걸림 032 · 052. 사슬 셋
]
...
#
# 아래 열여섯은 발화 옆 주석에 「함께 걸림」이 적힌 줄과 정확히 같다.
MARKS = {
    # 말한 것 — 열
    5: OVERLAP,    # 033 · 006 · 024
    6: OVERLAP,    # 005 · 025 · 035
    11: OVERLAP,   # 032 · 046 · 052
    13: OVERLAP,   # 016
    15: OVERLAP,   # 005 · 035
    17: OVERLAP,   # 006 · 025
    20: OVERLAP,   # 028 · 009 · 040
    23: OVERLAP,   # 012
    24: OVERLAP,   # 060
    29: OVERLAP,   # 013 · 056 · 060
    # 찍은 지점 — 하나
    31: OVERLAP,   # 026
    # 보이는 범위 — 다섯
    37: OVERLAP,   # 025
...
BASELINE_LAST = 29
EXTENSION_LAST = 36
BASELINE_LABEL = "말한 것 스물아홉"
EXTENSION_LABEL = "찍은 지점 일곱"
SCREEN_LABEL = "보이는 범위 아홉"
