...
UTTERANCES = [
    ...
    # bbox 뿐이면 resolve_service 가 menu 에서도 축 선택지에서도 뺀다.
    #
    # ★ **일곱이 다 「여기」로 시작한다. 베낀 것이 아니라 다른 수가 없다.**
    # 화면 recipe 는 SCREEN_WORDS 여덟 낱말 중 하나가 발화에 없으면 menu 에
    # 실리지도 않는다(resolve_service._points_at_screen). 그중 찍은 지점으로
    # 읽히는 것이 「여기 · 이 위치 · 선택한 위치 · 이 근처 · 이 지역 ·
    # 관심 지점」이고, 뒤의 셋은 보이는 범위로도 읽혀 갈래를 흐린다.
    # ★ 그래서 이 일곱은 **말투를 바꿀 수가 없다.** 바꾸면 아예 못 잰다.
    # 이것이 자의 한계이지 발화가 게으른 것이 아니다 — 「열린 과제」에 적었다.
    ...
    # 위와 같은 까닭이고 여기는 더 좁다 — SCREEN_WORDS 여덟 중 보이는 범위로
    # 읽히는 것이 그 **둘뿐**이다. 「화면에 보이는」·「화면에 걸친」은 부분
    # 문자열로 안 걸려 화면 recipe 를 아예 못 본다(실측으로 확인했다).
